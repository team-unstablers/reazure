...
**The post context menu** is built once in `PostContextMenuDescriptor.build` (`views/timeline/PostContextMenuDescriptor.swift`) and rendered twice: as SwiftUI `.contextMenu` content (`NativePostContextMenuInner`) and as a `UIMenu` for the `v` shortcut (`ProgrammaticContextMenuHost`). Handlers are bare closures with no view context, so anything needing a modal publishes a `PostRowPresentation` that `SetupContextMenuModifier` hosts. Entries are `.label` / `.action` / `.submenu`; the SwiftUI renderer returns `AnyView` because `.submenu` recurses.

The one submenu today lists **attached media** (`AttachmentAdaptor.altText` — Mastodon `description`, Misskey `comment`), numbered per kind ("사진 #1 / 동영상 #1"). Images request `.gallery`, which the modifier routes to a window or a `fullScreenCover` exactly as a thumbnail tap does; everything else opens in the browser, since the viewer is image-only. This is the only route to a post's media from the keyboard, and the only route at all from compact rows and for non-image attachments — none of which draw thumbnails. Picking an item deliberately skips the sensitive-media reveal: naming an attachment in a menu is already an explicit request to see it.
