...
class SharedClient: ObservableObject {
    ...
    private func constructTimelineModel() {
        let homeTimeline = TimelineModel(with: self) { [weak self] (args) in
            ...
            return statuses.map { StatusModel(adaptor: MastodonStatusAdaptor(from: $0)) }
        }
        ...
        let notificationsTimeline = TimelineModel(with: self) { [weak self] (args) in
            ...
            return notifications.compactMap { NotificationModel(adaptor: MastodonNotificationAdaptor(from: $0)) }
        }
        ...
        case "notification":
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    guard let model = NotificationModel(adaptor: adaptor) else {
                        return
                    }
